...
namespace S036_OrderSkdApp
{
    ...
    /// 月末出庫処理（部品振替）
    /// </summary>
    public partial class F205E_OutDataPartsExchange : BaseForm
    {
        #region ＜フィールド＞ 
        ...
        public F205E_OutDataPartsExchange(string fId) : base(fId)
        {
            InitializeComponent();
            this.titleLabel.Text = "月末出庫処理（部品振替）";
        }

        /// <summary>
        /// ロード処理
        /// </summary>
        private void F205E_OutDataPartsExchange_Load(object sender, EventArgs e)
        {
            try
            {
                // 月末
        ...
        private void supCodeC1ComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    var executeDate = result.Table.Rows[0].Field<string>("executeDate");
                    var isOk = DateTime.TryParse(executeDate, out var dt1);
                    if (!isOk)
                    {
                    ...
                    executeDateValueLabel.Text = executeDate ?? "";
                    executeDateValueLabel.Visible = true;
                    executeDateLabel.Visible = true;

                    delivDateC1DateEdit.Value = SansoBase.DatetimeFC.GetEndOfMonth(dt1);
                }
