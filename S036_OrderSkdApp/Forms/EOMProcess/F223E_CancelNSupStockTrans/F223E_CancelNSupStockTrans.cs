...
namespace S036_OrderSkdApp
{
    ...
    /// 月末返品有償受付
    /// </summary>
    public partial class F223E_CancelNSupStockTrans : BaseForm
    {
        #region ＜フィールド＞ 
        ...
        public F223E_CancelNSupStockTrans(string fId) : base(fId)
        {
            InitializeComponent();
            this.titleLabel.Text = "月末返品有償受付";
        }

        /// <summary>
        /// ロード処理
        /// </summary>
        private void F223E_CancelNSupStockTrans_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void DisplayClear()
        {
            ...
            outDateC1DateEdit.Value = endDate.ToShortDateString();
        ...
        private void outNumC1NumericEdit_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                if (d1 >= 0)
                {
                    ChangeTopMessage(1, "WARN", "数量がプラスまたはゼロで入力されています。マイナスで入力してください。");
                    ActiveControl = (C1TextBox)sender;
                    e.Cancel = true;
                    return;
                }
        ...
        private void SetStockInfo()
        {
            try
            {
                ...
                if (stockCateC1ComboBox.Text == "Z")
                {
                    ...
                    paramE.WhereColuList.Add((paramE.GroupCode, groupCodeC1ComboBox.Text));
        ...
        private bool SetGridViewInfo()
        {
            try
            {
                ...
                c1TrueDBGrid.SetDataBinding(dt, "", true);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error: {ex.Message}");
        ...
        public (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetGridViewData()
        {
            // 必要なパラメータ設定済
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F223E/GetIOProcessUnitPrice?sid={solutionIdShort}&fid={formIdShort}";
        ...
        public (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetGroupManufactDirect()
        {
            // 必要なパラメータ設定済
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F223E/GetGroupComboListByUser?sid={solutionIdShort}&fid={formIdShort}";
        ...
        public (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetStockCate()
        {
            // 必要なパラメータ設定済
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F223E/GetStockCate?sid={solutionIdShort}&fid={formIdShort}";
        ...
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetExecuteDate()
        {
            // 必要なパラメータ設定
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F223E/GetExecuteDate?sid={solutionIdShort}&fid={formIdShort}";
        ...
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetUnitPriceMst()
        {
            // 必要なパラメータ設定
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F223E/GetUnitPriceMst?sid={solutionIdShort}&fid={formIdShort}";
