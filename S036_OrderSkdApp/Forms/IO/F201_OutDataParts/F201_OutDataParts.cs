...
namespace S036_OrderSkdApp
{
    ...
    /// 出庫処理（部品）
    /// </summary>
    public partial class F201_OutDataParts : BaseForm
    {
        #region ＜起動処理＞ 
        ...
        public F201_OutDataParts(string fId) : base(fId)
        {
            InitializeComponent();
            this.titleLabel.Text = "出庫処理（部品）";
        }

        /// <summary>
        /// ロード処理
        /// </summary>
        private void F201_OutDataParts_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetGroupManufactDirect()
        {
            // 必要なパラメータ設定
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F201/GetGroupManufactDirect?sid={solutionIdShort}&fid={formIdShort}";
        ...
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetExecuteDate()
        {
            // 必要なパラメータ設定
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F201/GetExecuteDate?sid={solutionIdShort}&fid={formIdShort}";
