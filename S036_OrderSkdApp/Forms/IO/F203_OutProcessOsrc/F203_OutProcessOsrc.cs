...
namespace S036_OrderSkdApp
{
    ...
    /// 出庫処理 有償支給発行（外注）
    /// </summary>
    public partial class F203_OutProcessOsrc : BaseForm
    {
        ...
        public F203_OutProcessOsrc(string fId) : base(fId)
        {
            InitializeComponent();
            this.titleLabel.Text = "有償支給発行（外注）";
        }

        /// <summary>
        /// ロード処理
        /// </summary>
        private void F203_OutProcessOsrc_Load(object sender, EventArgs e)
        {
            try
            {
                // コントロールリストIIの要素
                ...
                AddControlListII(groupCodeC1TextBox, groupNameC1TextBox, "", true, enumCate.無し);
        ...
        private void ActionProc()
        {
            try
            {
                ...
                var doCodeCheck = doCodeC1CheckBox.Checked;
                apiParam.RemoveAll();
                ...
                apiParam.Add("nSupStockTransDataFlg", new JValue(doCodeCheck == true ? "" : "M"));
                ...
                apiParam.Add("doCodeCheck", new JValue(doCodeCheck));
        ...
        private void GetGroupCode()
        {
            ...
            var result2 = ApiCommonGet(apiUrl + "GetExecuteDate", apiParam);
            if (result2.IsOk == false)
            {
                ChangeTopMessage("E0008", "検索時に");
                return;
