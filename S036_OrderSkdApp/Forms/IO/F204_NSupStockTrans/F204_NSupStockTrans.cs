...
namespace S036_OrderSkdApp
{
    ...
    /// 有償支給発行（一般）
    /// </summary>
    public partial class F204_NSupStockTrans : BaseForm
    {
        #region ＜フィールド＞ 
        ...
        public F204_NSupStockTrans(string fId) : base(fId)
        {
            InitializeComponent();
            this.titleLabel.Text = "有償支給発行（一般）";
        }

        /// <summary>
        /// ロード処理
        /// </summary>
        private void F204_NSupStockTrans_Load(object sender, EventArgs e)
        {
            try
            {
                // コントロールリストIIの要素
        ...
        private bool SupCodeErrorCheck()
        {
            ...
            var af = new F204_NSupStockTransAF();
            var result2 = af.GetSansoMainAddressMst(t.Text);
            if (result2.IsOk == false)
            {
            ...
            var stopCate = result2.Table.Rows[0].Field<string>("取引停止区分");

            stopCate = stopCate.Substring(stopCate.Length - 1, 1) ?? "";
        ...
        {
            var af1 = new F204_NSupStockTransAF();
            var productName = "";
