...
namespace S036_OrderSkdApp
{
    ...
    /// 納入受付（伝票番号）
    /// </summary>
    public partial class F208_DelivAcceptanceDoCode : BaseForm
    {
        ...
        public F208_DelivAcceptanceDoCode(string fId) : base(fId)
        {
            InitializeComponent();
            this.titleLabel.Text = "納入受付（伝票番号）";
        }

        /// <summary>
        /// ロード処理
        /// </summary>
        private void F208_DelivAcceptanceDoCode_Load(object sender, EventArgs e)
        {
            try
            {
                // 左パネル
                AddControlListII(poCodeC1TextBox, null, "", false, enumCate.Key);    
                ...
                AddControlListII(inDateC1DateEdit, null, DateTime.Today.ToShortDateString(), true, enumCate.Key);
                ...
                AddControlListII(insideTransDateC1DateEdit, null, DateTime.Today.ToShortDateString(), false, enumCate.Key);
        ...
        /// クリア処理
        /// </summary>
        /// <param name="cate">入力無し or ""：伝票番号手打ち、入力有：伝票番号バーコード入力 </param>
        private void DisplayClear(string cate = "")
        {
            ...
            if (cate == "")
            {
                ...
                cancelC1CheckBox.Checked = false;
            }
            else
            {
                // バーコード入力 の場合、バーコードとキャンセルのチェックはクリア前と同じ
            }
        ...
        private void doCodeC1TextBox_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                if (isRunValidating == false || stDoCode == doCodeC1TextBox.Text)
                {
                    return;
                }

                // 伝票番号チェック
                var result1 = DoCodeErrorCheck();
                ...
                // 注文番号チェック
                var isOk2 = PoCodeErrorCheck(result1.PoCode);
                if (isOk2 == false)
                {
                    ActiveControl = (C1TextBox)sender;
                    e.Cancel = true;
        ...
        /// 伝票番号 検証後
        /// </summary>
        private void doCodeC1TextBox_Validated(object sender, EventArgs e)
        {
            try
            {
                if (isRunValidating == false)
                ...
                if (string.IsNullOrEmpty(t.Text) || stDoCode == t.Text)
                {
                    return;
                }

                poCodeC1TextBox.Text = t.Text.Substring(0, 10) ?? "";

                // 注文番号後処理
                var isOk1 = PoCodeValidated();
                if (isOk1 == false)
                {
                    ActiveControl = (C1TextBox)sender;
                    return;
                ...
                // EDIDATA.T_SHIPMENT_DATA
                var result1 = GetT_SHIPMENT_DATA();
                if (result1.IsOk == false)
                {
                ...
                {
                    ChangeTopMessage("W0002", "伝票番号", "現品票ファイル");
                    return;
                }

                inNumC1NumericEdit.Value = result1.Table.Rows[0].Field<double?>("deliveryQty").ToString() ?? "0";
                unitPriceC1NumericEdit.Value = result1.Table.Rows[0].Field<double?>("deliveryUp").ToString() ?? "0";
                inPriceC1NumericEdit.Value = result1.Table.Rows[0].Field<double?>("deliveryAmount").ToString() ?? "0";

                if (cancelC1CheckBox.Checked == true)
                {
                    var check1 = double.TryParse(inNumC1NumericEdit.Value.ToString(), out double num1);
                    inNumC1NumericEdit.Value = (num1 * -1).ToString() ?? "0";

                    var check2 = double.TryParse(inPriceC1NumericEdit.Value.ToString(), out double num2);
                    inPriceC1NumericEdit.Value = (num2 * -1).ToString() ?? "0";
                }

                // 発注マスタ 排他チェック用データ確保
                var paramOrd = new OrdMst();
                paramOrd.SelectStr = "*";
                paramOrd.WhereColuList.Add((paramOrd.PoCode, poCodeC1TextBox.Text));
        ...
        /// 取消チェックボックス 変更時
        /// </summary>
        private void cancelC1CheckBox_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(doCodeC1TextBox.Text))
                {
                    return;
                }
                ...
                if (result1.Table.Rows.Count <= 0)
                {
                    ChangeTopMessage(1, "WARN", "現品票ファイルにデータがありません。出荷済か確認してください");
                    return;
                }

                var status = result1.Table.Rows[0].Field<long?>("status") ?? 0;

                if (status == 8)
                {
                    MessageBox.Show($"この伝票は一度取消済ですが、再入力します", "再入力", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else if (status != 0)
                {
                    if (cancelC1CheckBox.Checked == true)
                    {
                        MessageBox.Show($"この伝票は処理済みですが、取消します", "取消", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        ChangeTopMessage(1, "WARN", "この伝票は処理済みです。処理できません");
                        return;
                    }
                }
                ...
                if ((cancelC1CheckBox.Checked == true && num1 >= 0)
                    || (cancelC1CheckBox.Checked == false && num1 <= 0))
                {
                    inNumC1NumericEdit.Value = (num1 * -1).ToString() ?? "0";
                    inPriceC1NumericEdit.Value = (num2 * -1).ToString() ?? "0";
                }
        ...
        private bool ErrCKF10()
        {
            ...
            var isOk2 = PoCodeErrorCheck(poCodeC1TextBox.Text);
            if (isOk2 == false)
            {
                return false;
            }

            var isOk3 = ApplicationCodeErrorCheck();
            if (isOk3 == false)
            {
            ...
            var isOk6 = InPriceErrorCheck();
            if (isOk6 == false)
            {
                return false;
            }

           var isOk7 = InDateErrorCheck();
            if (isOk7 == false)
            {
            ...
            var isOk9 = InsideTransDateErrorCheck();
            if (isOk9 == false)
            {
        ...
        private bool PrintProc()
        {
            ...
            if (nextProcessCodeC1TextBox.Text != "" && nextProcessC1CheckBox.Checked == true)
            {
                ...
                {
                    // 入出庫ファイル更新、帳票データ抽出処理
                    var resultIO = UpdateIOFilePrint();
                    ...
                    else
                    {
                        ...
                        var result = GetProductName(jyuyoyosokuCode);
                        if (result.IsOk == false)
                        {
                            return false;
                        }
                        var productNameIO = result.ProductName;
                ...
                if (nextProcessError == "E")
                {
                    ...
                    using (var report = new C1.Win.FlexReport.C1FlexReport())
                    {
                        ...
                        dt.Rows.Add(groupCodeC1TextBox.Text, poCodeC1TextBox.Text, supCodeC1TextBox.Text, supNameC1TextBox.Text,
            ...
            using (var report = new C1.Win.FlexReport.C1FlexReport())
            {
                ...
                if (print.IsOk == false)
                {
                    ChangeTopMessage("E0008", "次工程確認レポート 印刷処理で");
                    return false;
                }
            }
            return true;
        ...
        private (bool IsOk,string PoCode) DoCodeErrorCheck()
        {
            var t = doCodeC1TextBox;
            ...
            var isOk2 = Check.IsStringRange(t.Text, 13, 15);
            if (isOk2.Result == false)
            {
                ChangeTopMessage("W0009", t.Label.Text, "13", "15");
                return (false, "");
            }
            ...
            if (status == 8)
            {
                MessageBox.Show($"この伝票は一度取消済ですが、再入力します", "再入力", 
            ...
            else if (status != 0)
            {
                if (cancelC1CheckBox.Checked == true)
                {
                    MessageBox.Show($"この伝票は処理済みですが、取消します", "取消", 
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
            ...
            var poCode = result1.Table.Rows[0].Field<string>("poCode") ?? "";

            // 発注マスタ
            var paramOrd = new OrdMst();
            paramOrd.SelectStr = "*";
            paramOrd.WhereColuList.Add((paramOrd.PoCode, poCode));
            paramOrd.SetDBName("製造調達");
            var resultOrd = CommonAF.ExecutSelectSQL(paramOrd);
            var dtOrd = resultOrd.Table;
            if (resultOrd.IsOk == false)
            ...
            if (dtOrd.Rows.Count <= 0)
            {
                ChangeTopMessage("W0002", "データ", "発注マスタ");
                return (false, "");
            }
        ...
        /// </summary>
        /// <returns>True：エラーが無し False：エラーがある</returns>
        private bool PoCodeErrorCheck(string poCode)
        {
            ...
            var isOk1 = Check.HasBanChar(poCode).Result;
            if (isOk1 == false)
            {
                ChangeTopMessage("W0018");
                return false;
            ...
            var isOk2 = Check.IsStringRange(poCode, 10, 10);
            if (isOk2.Result == false)
            {
                ChangeTopMessage("W0009", "注文番号", "10", "10");
                return false;
            }
            ...
            var partsCode = dtOrd.Rows[0].Field<string>("部品コード") ?? "";
            var supCode = dtOrd.Rows[0].Field<string>("仕入先コード") ?? "";

            // 発注マスタから取得した部品コードの確認
            var paramParts = new PartsMst();
            paramParts.SelectStr = "*";
            paramParts.WhereColuList.Add((paramParts.PartsCode, partsCode));
            ...
            paramSup.SelectStr = "*";
            paramSup.WhereColuList.Add((paramSup.SupCode, supCode));
            ...
            var count = groupCodeDt.AsEnumerable().Where(v => v.Field<string>("groupCode") == groupCode).Count();
            ...
            // 住所録マスタ
            var resultAddress = SelectDBAF.GetSansoMainAddressMst(supCode);
            ...
            if (resultAddress.Table.Rows.Count <= 0)
            {
                ...
                if (temp.Count() <= 0)
                {
                    ChangeTopMessage("W0002", "課別コード","住所録マスタ、部門マスタ");
                    return false;
                }
            }
            else
            ...
            // 初品検査メッセージ 表示
            var result = GetFirstProdCheckOK(partsCode,supCode);
            if (result.IsOk == false)
            {
        ...
        private bool PoCodeValidated()
        {
            ...
            groupNameC1TextBox.Text = groupCodeDt.AsEnumerable()
                .Where(v => v.Field<string>("groupCode") == groupCodeC1TextBox.Text)
                .Select(v => v.Field<string>("groupName")).FirstOrDefault();
            ...
            inDateC1DateEdit.Text = DateTime.Today.ToShortDateString();
        ...
        private void SetStockInfo()
        {
            ...
            if (dt.Rows.Count >= 1)
            {
                prevMonthStockNumC1TextBox.Value = dt.Rows[0]["前残数量"].ToString().Replace(".000", "");
                thisMonthInNumC1TextBox.Value = dt.Rows[0]["入庫数量"].ToString().Replace(".000", "");
                thisMonthOutNumC1TextBox.Value = dt.Rows[0]["出庫数量"].ToString().Replace(".000", "");
                thisMonthStockNumC1TextBox.Value = dt.Rows[0]["当残数量"].ToString().Replace(".000", "");
            }
        }
        ...
        private void GetNextProcessInfo()
        {
            ...
            insideTransDateC1DateEdit.Value = DateTime.Today.ToShortDateString();
        ...
        /// </summary>
        /// <returns>True：エラーが無し False：エラーがある</returns>
        private bool InPriceErrorCheck()
        {
            // 納入金額の確認
        ...
        private (bool IsOk,string ProductName) GetProductName(string jyuyoyosokuCode)
        {
            var productName = "";
            if (jyuyoyosokuCode == "")
            {
                // 需要予測番号が入力無い場合、部品構成表から取得
                var result = GetBOMMst();
            ...
            else
            {
                ...
                var result = GetManufactFile(jyuyoyosokuCode);
                if (result.IsOk == false)
                {
                    if (result.ReLogin == true)
                    {
        ...
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetGroupComboListByUser()
        {
            ...
            url += $"Solution/S036/F208/GetGroupComboListByUser?sid={solutionIdShort}&fid={formIdShort}";
        ...
        /// Table：取得データ
        /// </returns>
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetT_SHIPMENT_DATA()
        {
            // 必要なパラメータ設定済
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"DeliveryNote/GetT_SHIPMENT_DATA?sid={solutionIdShort}&fid={formIdShort}";

            JObject param = new JObject();
            param.Add("doCode", new JValue(doCodeC1TextBox.Text ?? ""));
        ...
        /// Table：取得データ
        /// </returns>
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetFirstProdCheckOK(string partsCode,string supCode)
        {
            // 必要なパラメータ設定済
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/GetFirstProdCheckOK?sid={solutionIdShort}&fid={formIdShort}";
        ...
        private (bool IsOk, bool ReLogin, int Count, DataTable Table) FirstProdCheckApi(bool lot)
        {
            ...
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/GetFirstProdCheckMst?sid={solutionIdShort}&fid={formIdShort}";
        ...
        private (bool IsOk, bool ReLogin, DataTable Table) GetNextProcessInfoApi()
        {
            ...
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/GetNextProcessInfo?sid={solutionIdShort}&fid={formIdShort}";
        ...
        {
            // 必要なパラメータ設定
            var poCode = controlListII.SGetText("poCodeC1TextBox");

            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/GetPartsCheck?sid={solutionIdShort}&fid={formIdShort}";
        ...
        /// Msg：エラーメッセージ
        /// </returns>
        private (bool IsOk, bool ReLogin, string Msg) UpdateIOFile()
        {
            ...
            var isCancel = cancelC1CheckBox.Checked;
            
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/CreateDelivAcceptance?sid={solutionIdShort}&fid={formIdShort}";
            ...
            param.Add("isCancel", new JValue(isCancel));
        ...
        private (bool IsOk, bool ReLogin, string Msg) UpdateOrdDetail()
        {
            ...
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/UpdateManufactInstructionMst?sid={solutionIdShort}&fid={formIdShort}";
        ...
        /// Msg：エラーメッセージ
        /// </returns>
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) UpdateIOFilePrint()
        {
            ...
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/UpdateIOFilePrint?sid={solutionIdShort}&fid={formIdShort}";
        ...
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetBOMMst()
        {
            ...
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/GetBOMMst?sid={solutionIdShort}&fid={formIdShort}";
        ...
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetManufactFile(string jyuyoyosokuCode)
        {
            // 必要なパラメータ設定済
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/GetManufactFile?sid={solutionIdShort}&fid={formIdShort}";
        ...
        private (bool IsOk, bool ReLogin, string Msg, DataTable Table) GetProcessMst()
        {
            ...
            string url = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"];
            url += $"Solution/S036/F208/GetProcessMst?sid={solutionIdShort}&fid={formIdShort}";
