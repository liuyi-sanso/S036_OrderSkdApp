...
namespace S036_OrderSkdApp
{
    ...
    /// 納入データ検収
    /// </summary>
    public partial class F212_DelivBarcodeNotProcessList : BaseForm
    {
        #region ＜フィールド＞ 
        ...
        public F212_DelivBarcodeNotProcessList(string fId) : base(fId)
        {
            InitializeComponent();
            titleLabel.Text = "納入データ検収";
        }

        /// <summary>
        /// ロード処理
        /// </summary>
        private void F212_DelivBarcodeNotProcessList_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                defButtomMessage = "一覧の「部品コード」と「仕入先」と「次工程」と「伝票種類」と「移行年月日」" +
                    "(薄い黄色列)は直接変更できます。　   " +
                    "「削除」ボタン押下後、該当行のデータが削除されます。\n" +
                    "※エラー表示のある物は納入受付できません。　　" +
                    "※次工程と伝票種類を選択すると、納入受付後に伝票種類にしたがって出庫し、伝票が発行されます。";
        ...
        private void c1TrueDBGrid_BeforeColUpdate(object sender, C1.Win.C1TrueDBGrid.BeforeColUpdateEventArgs e)
        {
            try
            {
                ...
                if (caption != "部品コード" 
                    ...
                    && caption != "移行年月日")
                {
                    ChangeTopMessage(1, "ERR", "（「部品コード」と「仕入先」と「次工程」と「伝票種類」と「移行年月日」" +
                        "（薄黄色列）以外は更新できません");
                    e.Cancel = true;
                    return;
                }
