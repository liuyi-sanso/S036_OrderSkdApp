...
{
    public partial class F004_NoAcceptInfo : BaseForm
    {
        #region ＜フィールド＞ 
        /// <summary>
        /// WEBAPI接続先
        ...
        #region ＜起動処理＞ 
        public F004_NoAcceptInfo(string fId) : base(fId)
        {
            InitializeComponent();
            this.titleLabel.Text = "未受入問合せ";
        }

        private void F004_NoAcceptInfo_Load(object sender, EventArgs e)
        {
            try
            {
                //C1TextBoxをリスト化
                ...
                //C1DateEditをリスト化
                AddControlListII(startDateC1DateEdit, null, DateTime.Now.AddDays(-1).ToString(), true, enumCate.無し);
        ...
        private void ComboBoxSelectIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (groupCodeC1ComboBox.Text == "3734")
                {
                    // 現品票番号は非表示
                    c1TrueDBGrid.Splits[0].DisplayColumns["現品票番号"].Visible = false;
                }

                if (groupCodeC1ComboBox.Text == "3730")
                {
                    // 現品票番号は非表示
                    c1TrueDBGrid.Splits[0].DisplayColumns["現品票番号"].Visible = true;
                }
        ...
        private void ActionProc()
        {
            try
            {
                ...
                var result1 = ApiCommonGet(apiUrl + "Solution/S036/F004/GetNoAccept3734", apiParam);

                if (groupCodeC1ComboBox.Text == "3734")
                {
                     result1 = ApiCommonGet(apiUrl + "Solution/S036/F004/GetNoAccept3734", apiParam);
                }
                else
                {
                     result1 = ApiCommonGet(apiUrl + "Solution/S036/F004/GetNoAccept3730", apiParam);
                }
        ...
        private void EXCELProc()
        {
            ...
            //param.Add((1, "", 2400));
            param.Add((1, "yyyy/m/d;@", 1200));
            param.Add((14, "yyyy/m/d hh:mm:ss;@", 1200));
            param.Add((16, "yyyy/m/d hh:mm:ss;@", 1200));
