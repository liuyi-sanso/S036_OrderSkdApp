...
namespace S036_OrderSkdApp
{
    ...
    /// 不適合品問合せ画面
    /// </summary>
    public partial class F005_FailedInfo : BaseForm
    {
        #region ＜フィールド＞ 
        /// <summary>
        /// WEBAPI接続先
        /// </summary>
        private string apiUrl = System.Configuration.ConfigurationManager.AppSettings["connectionWebAPIURL"] + "Solution/S036/F005/";
        ...
        /// コンストラクタ
        /// </summary>
        public F005_FailedInfo(string fId) : base(fId)
        {
            InitializeComponent();
            this.titleLabel.Text = "不適合品問合せ";
        }

        /// <summary>
        /// ロード処理
        /// </summary>
        private void F005_FailedInfo_Load(object sender, EventArgs e)
        {
            try
            {
                //C1TextBoxをリスト化
        ...
        private void DisplayClear()
        {
            ...
            c1TrueDBGridDT = null;
            completeCateC1ComboBox.SelectedIndex = 0;
        ...
        /// 課別コード 検証中
        /// </summary>
        private void groupCodeC1TextBox_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                if (isRunValidating == false)
        ...
        /// 課別コード 検証後
        /// </summary>
        private void groupCodeC1TextBox_Validated(object sender, EventArgs e)
        {
            try
            {
                if (isRunValidating == false)
                ...
                groupCodeNameC1TextBox.Text = "";

                // 未入力時処理
                var s = groupCodeC1TextBox;
                if (string.IsNullOrEmpty(s.Text))
                {
                    return;
                }
                ...
                // 仕入先マスタ
                apiParam.RemoveAll();
                apiParam.Add("supCode", new JValue(groupCodeC1TextBox.Text));
                var result = CallSansoWebAPI("POST", apiUrl + "GetSupMst", apiParam);
                if (result.IsOk == false)
                {
                    return;
        ...
        private bool ErrCKF10()
        {
            ...
            var isOk2 = ErrorCheckGroupCode();
            if (isOk2 == false)
            {
                ActiveControl = groupCodeC1TextBox;
                return false;
            }
        ...
        private void ActionProc()
        {
            try
            {
                ...
                apiParam.Add("supCode", new JValue(supCodeC1TextBox.Text));
                apiParam.Add("partsCode", new JValue(partsCodeC1TextBox.Text));
                apiParam.Add("completeCate", new JValue(completeCateC1ComboBox.SGetText(1)));
                ...
                if (result.IsOk == false)
                {
                    ChangeTopMessage("E0008", "不適合品一覧抽出時に");
                    return;
                }
        ...
        private void EXCELProc()
        {
            ...
            table.Columns["prevMonthStockNum"].ColumnName = "前残数量";
            table.Columns["prevMonthStockPrice"].ColumnName = "前残金額";
            table.Columns["inNum"].ColumnName = "入庫数量";
            table.Columns["inPrice"].ColumnName = "入庫金額";
            table.Columns["outNum"].ColumnName = "出庫数量";
            table.Columns["outPrice"].ColumnName = "出庫金額";
            table.Columns["thisMonthStockNum"].ColumnName = "当残数量";
            table.Columns["thisMonthStockPrice"].ColumnName = "当残金額";
            table.Columns["lastInDate"].ColumnName = "最終入庫日";
            table.Columns["lastOutDate"].ColumnName = "最終出庫日";
            table.Columns["createDate"].ColumnName = "作成日付";
            table.Columns["updateDate"].ColumnName = "変更日付";
            table.Columns["partsName"].ColumnName = "部品名";
            table.Columns["partsCate"].ColumnName = "部品区分";
            table.Columns["qualityStaffCode"].ColumnName = "品管担当者コード";
            table.Columns["qualityJudgment"].ColumnName = "判定";
            table.Columns["qualityRemarks"].ColumnName = "備考";
            table.Columns["qualityJudgmentDate"].ColumnName = "判定日付";
            table.Columns["staffCode"].ColumnName = "資材担当者コード";
            table.Columns["judgment"].ColumnName = "資材判定";
            table.Columns["remarks"].ColumnName = "資材備考";
            table.Columns["judgmentDate"].ColumnName = "資材判定日付";
            table.Columns["qualityStaff"].ColumnName = "品管担当者";
            table.Columns["staff"].ColumnName = "資材担当者";
            table.Columns["groupName"].ColumnName = "部門名";
            table.Columns["failedContent"].ColumnName = "不適合内容";
            table.Columns["completeFlg"].ColumnName = "完了Ｆ";
            table.Columns["jyuyoyosokuCode"].ColumnName = "工事番号";
            table.Columns["supCode"].ColumnName = "返品先コード";
            table.Columns["supName"].ColumnName = "返品先名";
            table.Columns["drawingCode"].ColumnName = "図面番号";
            table.Columns["createID"].ColumnName = "作成者ID";
            table.Columns["updateID"].ColumnName = "変更者ID";
            ...
            //param.Add((26, "yyyy/MM/dd HH:mm:ss", 2400));
        ...
        /// </summary>
        /// <returns>True：エラーが無し False：エラーがある</returns>
        private bool ErrorCheckGroupCode()
        {
            // 未入力時処理
            var s = groupCodeC1TextBox;
            if (string.IsNullOrEmpty(s.Text))
